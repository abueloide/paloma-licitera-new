...
class DOFLicitacionesExtractor:
    ...
    def extraer_licitacion_de_bloque(self, texto: str, num_pagina: int) -> Optional[Licitacion]:
        ...
        patrones = {
            ...
            'caracter': [
                r"(?:Car[áa]cter(?:\s+de\s+la\s+[Ll]icitaci[óo]n)?)[:\s]*([^\n]+)",
            ...
            'objeto': [
                ...
                r"(?:Nombre\s*del\s*Procedimiento\s*de\s*contrataci[óo]n)[:\s\.]*([^\n]+)"
            ],
            'volumen': [
                r"(?:Volumen\s*a\s*[Aa]dquirir)[:\s\.]*([^\n]+)",
                r"(?:Cantidad|N[úu]mero\s*de\s*[Ss]ervicios)[:\s\.]*([^\n]+)"
            ],
            'fecha_publicacion': [
                r"(?:Fecha\s*de\s*[Pp]ublicaci[óo]n(?:\s*en\s*Compras?\s*M[Xx])?)[:\s\.]*([^\n]+)",
                r"(?:Publicaci[óo]n\s*en\s*Compras?\s*M[Xx])[:\s\.]*([^\n]+)"
            ],
            'fecha_junta': [
                r"(?:Junta\s*de\s*[Aa]claraciones?)[:\s\.]*([^\n]+)",
                r"(?:Fecha\s*y\s*hora\s*de\s*junta\s*de\s*aclaraciones)[:\s\.]*([^\n]+)"
            ],
            'fecha_visita': [
                r"(?:Visita\s*a\s*(?:las\s*)?[Ii]nstalaciones?)[:\s\.]*([^\n]+)",
            ],
            'fecha_presentacion': [
                r"(?:Presentaci[óo]n\s*y\s*[Aa]pertura\s*de\s*[Pp]roposiciones?)[:\s\.]*([^\n]+)",
                r"(?:Apertura\s*de\s*[Pp]roposiciones?)[:\s\.]*([^\n]+)"
            ],
            'fecha_fallo': [
                r"(?:Fallo|Emisi[óo]n\s*de\s*[Ff]allo|Fecha\s*(?:y\s*hora\s*)?de\s*[Ff]allo)[:\s\.]*([^\n]+)",
